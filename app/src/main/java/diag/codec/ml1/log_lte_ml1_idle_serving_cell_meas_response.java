...
/**
 * Created by xxx on 2018/3/5.0xB193
 */

public class log_lte_ml1_idle_serving_cell_meas_response {
    private static String subpacket_name[] = {
            ...
            "23",
            "24",
            "25 - Serving Cell Measurement Result",
    ...
    private static log_packet_field[] field_fmt = {
            ...
            new log_packet_field(FIELD_TYPE_INT, "SubPacket ID", 4, 1),
    };

    static public void decode(log_packet packet) {
        System.out.println("log_lte_ml1_idle_serving_cell_meas_response");
        packet.log_name = "log_lte_ml1_idle_serving_cell_meas_response";

        packet.add_fields(field_fmt);

        log_packet_field subPacketId = packet.get_field("SubPacket ID");

        if (subPacketId.value_int != 25) {
        ...
        packet.add_field(new log_packet_field(FIELD_TYPE_INT, "SubPacket Version", 5, 1));
        packet.add_field(new log_packet_field(FIELD_TYPE_INT, "SubPacket Size", 6, 2));
        packet.add_field(new log_packet_field(FIELD_TYPE_INT, "SubPacket E-ARFCN", 8, 4));
        packet.add_field(new log_packet_field(FIELD_TYPE_INT, "SubPacket PCI", 16, 2, 0, 12, TO_VOID));
        packet.add_field(new log_packet_field(FIELD_TYPE_DOUBLE, "Inst RSRP", 40, 2, 0, 12, TO_RSRP));
        packet.add_field(new log_packet_field(FIELD_TYPE_DOUBLE, "Inst RSRQ", 48, 2, 0, 10, TO_RSRQ));
    }
}
