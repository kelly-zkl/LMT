...
/**
 * Created by xxx on 2018/3/20.0xB192
 */

public class log_lte_ml1_idle_neighbor_cell_meas_request_response {
    private static String subpacket_name[] = {
            ...
            "26 - Idle Mode Neighbor Cell Measurement Request",
            "27 - Idle Mode Neighbor Cell Meas Result"
    };
    ...
    private static log_packet_field[] field_fmt = {
            ...
            new log_packet_field(FIELD_TYPE_INT, "SubPacket ID", 32, 1),
    };

    static public void decode(log_packet packet) {
        System.out.println("log_lte_ml1_idle_neighbor_cell_meas_request_response");
        packet.log_name = "log_lte_ml1_idle_neighbor_cell_meas_request_response";

        packet.add_fields(field_fmt);

        log_packet_field subPacketId = packet.get_field("SubPacket ID");

        if (subPacketId.value_int != 27) {
        ...
        packet.add_field(new log_packet_field(FIELD_TYPE_INT, "SubPacket E-ARFCN", 36, 2));
        packet.add_field(new log_packet_field(FIELD_TYPE_INT, "SubPacket PCI", 44, 2, 0, 12, TO_VOID));
        packet.add_field(new log_packet_field(FIELD_TYPE_DOUBLE, "Inst RSRP", 48, 2, 0, 12, TO_RSRP));
        packet.add_field(new log_packet_field(FIELD_TYPE_DOUBLE, "Inst RSRQ", 56, 2, 0, 10, TO_RSRQ));
    }
}
